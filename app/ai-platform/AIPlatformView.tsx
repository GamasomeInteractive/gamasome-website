...
export default function AIPlatformView(props: Props) {
  const { data: rawPageD } = useTina({
    data: props.pageData,
    query: props.pageQuery,
    variables: props.pageVars,
  })
  const { data: rawHeaderD } = useTina({
    data: props.headerData,
    query: props.headerQuery,
    variables: props.headerVars,
  })
  const { data: rawFooterD } = useTina({
    data: props.footerData,
    query: props.footerQuery,
    variables: props.footerVars,
  })
  ...
  const {
    ...
    costs,
    costsOutro,
    costsMultimodalTwoCol,
    comparisonLabel,
    comparisonTitle,
    comparisonDescription,
    comparison,
    comparisonAfterExamples,
    howItWorksLabel,
    howItWorksTitle,
  ...
  const useCasesSection = (
    ...
    >
      <div className="container mx-auto px-6 md:px-16">
        <FadeIn>
          <p
            className="gama-label mb-3 text-xs font-bold tracking-[0.2em] uppercase"
            data-tina-field={tinaField(page, 'useCasesLabel')}
          >
            {useCasesLabel}
          </p>
          <h2
            className="max-w-xl text-4xl font-bold md:text-5xl"
            data-tina-field={tinaField(page, 'useCasesTitle')}
          >
            {useCasesTitle}
          </h2>
        </FadeIn>
        <StaggerContainer
          className={`mt-14 grid grid-cols-1 gap-6 sm:grid-cols-2 ${useCases?.length === 4 ? 'lg:grid-cols-4' : 'lg:grid-cols-3'}`}
          staggerDelay={0.08}
        >
          {useCases?.map((uc: any, i: number) => (
            ...
            >
              <div
                className="absolute right-0 bottom-0 left-0 h-0.5 opacity-50 group-hover:opacity-100"
              ...
              <h3 className="mb-3 text-lg font-bold" data-tina-field={tinaField(uc, 'title')}>
                {uc.title}
              </h3>
              <p
                className="gama-body text-sm leading-relaxed"
                data-tina-field={tinaField(uc, 'description')}
              >
        ...
        <div className="container mx-auto px-6 md:px-16">
          <FadeIn>
            {comparisonLabel && (
              <p
                className="gama-label mb-3 text-xs font-bold tracking-[0.2em] uppercase"
                data-tina-field={tinaField(page, 'comparisonLabel')}
              >
                {comparisonLabel}
              </p>
            )}
            <h2
              className="max-w-3xl text-4xl font-bold md:text-5xl"
              data-tina-field={tinaField(page, 'comparisonTitle')}
            >
              {comparisonTitle}
            </h2>
            {comparisonDescription && (
              <p
                className="gama-body mt-4 max-w-3xl text-base"
                data-tina-field={tinaField(page, 'comparisonDescription')}
              >
                {comparisonDescription}
              </p>
            )}
              ...
              <thead>
                <tr style={{ background: bgSecondary }}>
                  {comparison.headers?.map((h: string, i: number) => (
                    <th
                      key={i}
                      className={`px-6 py-5 text-left text-sm font-bold tracking-wide uppercase ${i === 1 ? '' : 'text-white/70'}`}
                      style={i === 1 ? { color: accentColor } : undefined}
                    >
                      {h}
                    </th>
                  ))}
              ...
              <tbody>
                {comparison.rows.map((row: any, i: number) => (
                  <tr key={i} className={i % 2 === 0 ? 'bg-white/[0.02]' : 'bg-transparent'}>
                    <td className="border-t border-white/[0.06] px-6 py-5 text-sm font-semibold text-white">
                      {row.activity}
                    </td>
                    <td
                      className="border-t border-white/[0.06] px-6 py-5 text-sm font-semibold"
                      style={{ color: accentColor }}
                    >
                      {row.gamasome}
                    </td>
                    <td className="border-t border-white/[0.06] px-6 py-5 text-sm text-white/55">
                      {row.vendors}
                    </td>
                  </tr>
                ))}
  ...
  return (
    ...
    >
      ...
      <div className="w-full">
        ...
        >
          <div
            ...
            style={{
              background: `radial-gradient(ellipse 80% 55% at 60% 40%, ${primaryColor}24 0%, transparent 65%)`,
            }}
          />
          <div
          ...
          <div
            className="pointer-events-none absolute top-28 right-4 z-0 overflow-hidden sm:top-32 sm:right-6 lg:top-1/2 lg:right-10 lg:-translate-y-1/2"
            style={{
          ...
          <div className="relative z-10 container mx-auto px-6 pt-40 pb-20 md:px-16 lg:w-[80%]">
            <SlideFromLeft>
              <div
              ...
              >
                ...
                />
                <span
                  className="gama-label text-xs font-bold tracking-[0.2em] uppercase"
              ...
              <h1 className="max-w-2xl text-5xl leading-[1.1] font-bold md:text-6xl lg:text-7xl">
                <span data-tina-field={tinaField(hero, 'headline')}>{hero?.headline}</span> <br />
                <span style={gradientText} data-tina-field={tinaField(hero, 'headlineAccent')}>
                  {hero?.headlineAccent}
                </span>
              </h1>
              <p
                className="gama-body mt-7 max-w-xl text-base leading-relaxed md:text-lg"
        ...
        {stats && stats.length > 0 && (
          <section
            ...
            style={{ background: bgStats }}
            data-tina-field={tinaField(page, 'typography')}
          >
              ...
              <div className="grid grid-cols-2 gap-10 md:grid-cols-4">
                {stats.map((stat: any, i: number) => (
                  <FadeIn key={i} delay={i * 0.1}>
                    ...
                    >
                      <div
                        className="flex min-h-[2.5em] items-start text-4xl leading-tight font-bold md:text-5xl"
                        style={gradientText}
                      >
                        {stat.value}
                      </div>
                      {stat.descHtml ? (
                        <p
                          className="mt-3 text-sm leading-relaxed text-white/45 [&_a]:font-medium [&_a]:text-[#4c8dff] [&_a]:underline"
                          data-tina-field={tinaField(stat, 'descHtml')}
                          dangerouslySetInnerHTML={{ __html: stat.descHtml }}
                        />
                      ) : (
                        <>
                          <div className="mt-3 min-h-[2.5em] text-base font-semibold text-white">
                            {stat.label}
                          </div>
                ...
                <FadeIn>
                  <div>
                    {directAnswerLabel && (
                      <p
                        className="gama-label mb-3 text-xs font-bold tracking-[0.2em] uppercase"
                        data-tina-field={tinaField(page, 'directAnswerLabel')}
                      >
                        {directAnswerLabel}
                      </p>
                    )}
                    <h2
                      ...
                      data-tina-field={tinaField(page, 'directAnswerTitle')}
                    >
                      {directAnswerTitle}
                    </h2>
                    <div className="mt-7">
                      {directAnswerBody?.map((para: any, i: number) => (
                        <p key={i} className="gama-body mb-4 text-base leading-relaxed">
                      ...
                      <ul className="flex flex-col gap-3">
                        {directAnswerChecklist.map((item: any, i: number) => (
                          <li key={i} className="flex items-start gap-3">
                            ...
                            <span className="gama-body text-[15px] leading-relaxed">
            ...
            <div className="container mx-auto px-6 md:px-16">
              <FadeIn>
                {problemsLabel && (
                  <p
                    className="gama-label mb-3 text-xs font-bold tracking-[0.2em] uppercase"
                    data-tina-field={tinaField(page, 'problemsLabel')}
                  >
                    {problemsLabel}
                  </p>
                )}
                <h2
                  className="max-w-3xl text-4xl font-bold md:text-5xl"
                  data-tina-field={tinaField(page, 'problemsTitle')}
                >
                  {problemsTitle}
                </h2>
                {problemsDescription && (
                  <p
                    className="gama-body mt-4 max-w-3xl text-base"
                    data-tina-field={tinaField(page, 'problemsDescription')}
                  >
                    {problemsDescription}
                  </p>
                )}
              ...
              >
                {problems.map((p: any, i: number) => (
                  <StaggerItem
                    key={i}
                    className="group relative rounded-2xl border border-white/[0.08] bg-white/[0.03] p-7 transition-all duration-300 hover:border-white/20 hover:bg-white/[0.06]"
                  >
                    {p.icon && (
                      <div className="mb-4 text-3xl" data-tina-field={tinaField(p, 'icon')}>
                        {p.icon}
                      </div>
                    )}
                    <h3 className="mb-2 text-lg font-bold" data-tina-field={tinaField(p, 'title')}>
                      {p.title}
                    </h3>
                    <p
                      className="gama-body text-sm leading-relaxed [&_a]:font-medium [&_a]:text-[#4c8dff] [&_a]:underline"
            ...
            <div className="container mx-auto px-6 md:px-16">
              <FadeIn>
                {methodsLabel && (
                  <p
                    className="gama-label mb-3 text-xs font-bold tracking-[0.2em] uppercase"
                    data-tina-field={tinaField(page, 'methodsLabel')}
                  >
                    {methodsLabel}
                  </p>
                )}
                <h2
                  className="max-w-3xl text-4xl font-bold md:text-5xl"
                  data-tina-field={tinaField(page, 'methodsTitle')}
                >
                  {methodsTitle}
                </h2>
                {methodsDescription && (
                  <p
                    className="gama-body mt-4 max-w-3xl text-base"
                    data-tina-field={tinaField(page, 'methodsDescription')}
                  >
                    {methodsDescription}
                  </p>
                )}
                  ...
                  <thead>
                    <tr style={{ background: bgSecondary }}>
                      {methodsTable.headers?.map((h: string, i: number) => (
                        <th
                          key={i}
                          className="px-6 py-5 text-left text-sm font-bold tracking-wide uppercase"
                          style={{ color: accentColor }}
                        >
                          {h}
                        </th>
                  ...
                  <tbody>
                    {methodsTable.rows.map((row: any, i: number) => (
                      <tr key={i} className={i % 2 === 0 ? 'bg-white/[0.02]' : 'bg-transparent'}>
                        ...
                        <td className="border-t border-white/[0.06] px-6 py-5 align-top text-sm text-white/70">
                          {row.bestFor}
                        </td>
                        <td className="border-t border-white/[0.06] px-6 py-5 align-top text-sm text-white/70">
                          {row.fidelity}
              ...
              {methodsFootnote && (
                <p
                  className="gama-body mt-6 text-[13.5px] [&_a]:font-medium [&_a]:text-[#4c8dff] [&_a]:underline"
                  data-tina-field={tinaField(page, 'methodsFootnote')}
                  dangerouslySetInnerHTML={{ __html: methodsFootnote }}
                />
              )}
            </div>
          </section>
            ...
            {howItWorksPipeline ? (
              <>
                ...
                <FadeIn>
                  {howItWorksLabel && (
                    <p
                      className="gama-label mb-3 text-xs font-bold tracking-[0.2em] uppercase"
                      data-tina-field={tinaField(page, 'howItWorksLabel')}
                    >
                      {howItWorksLabel}
                    </p>
                  )}
                  <h2
                    className="max-w-3xl text-4xl font-bold md:text-5xl"
                    data-tina-field={tinaField(page, 'howItWorksTitle')}
                  >
                    {howItWorksTitle}
                  </h2>
                  {howItWorksDescription && (
                    <p
                      className="gama-body mt-4 max-w-3xl text-base"
                      data-tina-field={tinaField(page, 'howItWorksDescription')}
                    >
                      {howItWorksDescription}
                    </p>
                  )}
                ...
                <StaggerContainer
                  className="mt-12 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6"
                  staggerDelay={0.06}
                >
                  {howItWorks?.map((step: any, i: number) => (
                    <StaggerItem
                      key={i}
                      className="rounded-xl border border-white/[0.1] bg-white/[0.03] p-5"
                    >
                      <span
                      ...
                      <p
                        className="gama-body text-[13.5px] leading-relaxed"
              ...
              <>
                <FadeIn className="text-center">
                  ...
                  {howItWorksDescription && (
                    <p
                      className="gama-body mx-auto mt-4 max-w-3xl text-base"
                      data-tina-field={tinaField(page, 'howItWorksDescription')}
                    >
                      {howItWorksDescription}
                    </p>
                ...
                <div className="relative mt-20">
                  <div
                    className="absolute -top-10 right-[18%] left-[18%] hidden h-px md:block"
                    style={{
                      background: `linear-gradient(to right, transparent, ${primaryColor}4D, transparent)`,
                    }}
                  />
                  <div
                    className={`grid grid-cols-1 gap-12 ${howItWorks && howItWorks.length > 3 ? 'md:grid-cols-3 lg:grid-cols-3' : 'md:grid-cols-3'}`}
                  >
                    {howItWorks?.map((step: any, i: number) => (
                      <SlideFromBottom key={i} delay={i * 0.1}>
                        <div className="text-center">
                          ...
                          <p
                            className="gama-body mx-auto max-w-xs text-sm leading-relaxed"
                            data-tina-field={tinaField(step, 'description')}
                          >
                            {typeof step.description === 'string'
                              ? step.description
          ...
          <SectionMotion motion={capabilitiesMotion} as="section" className="py-16">
            <div className="container mx-auto px-6 md:px-16">
              <FadeIn>
                <p
                  className="gama-label mb-3 text-xs font-bold tracking-[0.2em] uppercase"
                  data-tina-field={tinaField(page, 'capabilitiesLabel')}
                >
                  {capabilitiesLabel}
                </p>
                <h2
                ...
                {capabilitiesDescription && (
                  <p
                    className="gama-body mt-4 max-w-3xl text-base"
                    data-tina-field={tinaField(page, 'capabilitiesDescription')}
                  >
                    {capabilitiesDescription}
                  </p>
                )}
              ...
              >
                {capabilities?.map((cap: any, i: number) => (
                  <StaggerItem
                    key={i}
                    className="group relative rounded-2xl border border-white/[0.08] bg-white/[0.03] p-8 transition-all duration-300 hover:border-white/20 hover:bg-white/[0.06]"
                  >
                    <div
                      className="absolute top-8 bottom-8 left-0 w-0.5 rounded-full opacity-0 transition-opacity duration-300 group-hover:opacity-100"
                      style={{
            ...
            <div className="container mx-auto px-6 md:px-16">
              <FadeIn>
                {qualityLabel && (
                  <p
                    className="gama-label mb-3 text-xs font-bold tracking-[0.2em] uppercase"
                    data-tina-field={tinaField(page, 'qualityLabel')}
                  >
                    {qualityLabel}
                  </p>
                )}
                <h2
                  className="max-w-3xl text-4xl font-bold md:text-5xl"
                  data-tina-field={tinaField(page, 'qualityTitle')}
                >
                  {qualityTitle}
                </h2>
                {qualityDescription && (
                  <p
                    className="gama-body mt-4 max-w-2xl text-lg"
                    data-tina-field={tinaField(page, 'qualityDescription')}
                  >
                    {qualityDescription}
                  </p>
                )}
                ...
                <FadeIn>
                  <div>
                    {qualityList.map((q: any, i: number) => (
                      <div
                        key={i}
                        className={`grid grid-cols-[auto_1fr] gap-5 py-4 ${i > 0 ? 'border-t border-white/[0.08]' : ''}`}
                      >
                        <span
                          className="pt-0.5 font-mono text-[13px] font-semibold tracking-wider whitespace-nowrap"
                          style={{ color: accentColor }}
                          data-tina-field={tinaField(q, 'score')}
                        >
                          {q.score}
                        </span>
                        <div>
                          <h3
                            ...
                            data-tina-field={tinaField(q, 'title')}
                          >
                            {q.title}
                          </h3>
                          <p
                            className="gama-body text-sm leading-relaxed"
                            data-tina-field={tinaField(q, 'description')}
                          >
                            {q.description}
                          </p>
                        </div>
                    ...
                    <div className="rounded-2xl border border-white/[0.08] bg-white/[0.03] p-8">
                      {qualityCardTitle && (
                        <h3
                          ...
                          data-tina-field={tinaField(page, 'qualityCardTitle')}
                        >
                          {qualityCardTitle}
                        </h3>
                      )}
            ...
            <div className="container mx-auto px-6 md:px-16">
              <FadeIn>
                {securityLabel && (
                  <p
                    className="gama-label mb-3 text-xs font-bold tracking-[0.2em] uppercase"
                    data-tina-field={tinaField(page, 'securityLabel')}
                  >
                    {securityLabel}
                  </p>
                )}
                <h2
                  className="max-w-3xl text-4xl font-bold md:text-5xl"
                  data-tina-field={tinaField(page, 'securityTitle')}
                >
                  {securityTitle}
                </h2>
                {securityDescription && (
                  <p
                    className="gama-body mt-4 max-w-3xl text-base"
                    data-tina-field={tinaField(page, 'securityDescription')}
                  >
                    {securityDescription}
                  </p>
                )}
              ...
              >
                {security.map((s: any, i: number) => (
                  <StaggerItem
                    key={i}
                    className="flex items-start gap-3 rounded-xl border border-white/[0.08] bg-white/[0.03] p-5"
                  >
                    <span
            ...
            <div className="container mx-auto px-6 md:px-16">
              <FadeIn>
                {pitfallsLabel && (
                  <p
                    className="gama-label mb-3 text-xs font-bold tracking-[0.2em] uppercase"
                    data-tina-field={tinaField(page, 'pitfallsLabel')}
                  >
                    {pitfallsLabel}
                  </p>
                )}
                <h2
                  className="max-w-3xl text-4xl font-bold md:text-5xl"
                  data-tina-field={tinaField(page, 'pitfallsTitle')}
                >
                  {pitfallsTitle}
                </h2>
                {pitfallsDescription && (
                  <p
                    className="gama-body mt-4 max-w-3xl text-base"
                    data-tina-field={tinaField(page, 'pitfallsDescription')}
                  >
                    {pitfallsDescription}
                  </p>
                )}
              ...
              >
                {pitfalls.map((p: any, i: number) => (
                  <StaggerItem
                    ...
                    className="group relative rounded-2xl border border-white/[0.08] bg-white/[0.03] p-8 transition-all duration-300 hover:border-white/20 hover:bg-white/[0.06]"
                  >
                    <h3 className="mb-3 text-lg font-bold" data-tina-field={tinaField(p, 'title')}>
                      {p.title}
                    </h3>
                    <p
                      className="gama-body text-sm leading-relaxed [&_a]:font-medium [&_a]:text-[#4c8dff] [&_a]:underline"
            ...
            <div className="container mx-auto px-6 md:px-16">
              <FadeIn>
                {examplesLabel && (
                  <p
                    className="gama-label mb-3 text-xs font-bold tracking-[0.2em] uppercase"
                    data-tina-field={tinaField(page, 'examplesLabel')}
                  >
                    {examplesLabel}
                  </p>
                )}
                <h2
                  className="max-w-3xl text-4xl font-bold md:text-5xl"
                  data-tina-field={tinaField(page, 'examplesTitle')}
                >
                  {examplesTitle}
                </h2>
                {examplesDescription && (
                  <p
                    className="gama-body mt-4 max-w-3xl text-base"
                    data-tina-field={tinaField(page, 'examplesDescription')}
                  >
                    {examplesDescription}
                  </p>
                )}
              ...
              >
                {examples.map((ex: any, i: number) => (
                  <StaggerItem
                    key={i}
                    className="rounded-2xl border border-white/[0.08] bg-white/[0.03] p-8 transition-all duration-300 hover:border-white/20 hover:bg-white/[0.06]"
                  >
                    <h3 className="mb-6 text-lg font-bold" data-tina-field={tinaField(ex, 'title')}>
                      {ex.title}
                    </h3>
                    <dl className="flex flex-col gap-4">
                      {(ex.steps ?? []).map((s: any, j: number) => (
                        <div key={j}>
                          <dt
                            className="mb-1 text-[11px] font-bold tracking-[0.12em] uppercase"
                            style={{ color: accentColor }}
                          ...
                          <dd
                            className="gama-body text-sm leading-relaxed"
                            data-tina-field={tinaField(s, 'text')}
                          >
                            {s.text}
                          </dd>
                        </div>
                ...
                <FadeIn>
                  <div>
                    {costsLabel && (
                      <p
                        className="gama-label mb-3 text-xs font-bold tracking-[0.2em] uppercase"
                        data-tina-field={tinaField(page, 'costsLabel')}
                      >
                        {costsLabel}
                      </p>
                    )}
                    <h2
                      ...
                      data-tina-field={tinaField(page, 'costsTitle')}
                    >
                      {costsTitle}
                    </h2>
                    {costsIntro && (
                      <p
                        className="gama-body mt-4 text-base [&_a]:font-medium [&_a]:text-[#4c8dff] [&_a]:underline"
                        data-tina-field={tinaField(page, 'costsIntro')}
                        dangerouslySetInnerHTML={{ __html: costsIntro }}
                      />
                    )}
                    ...
                    {costsOutro && (
                      <p
                        className="gama-body mt-8 text-base [&_a]:font-medium [&_a]:text-[#4c8dff] [&_a]:underline"
                        data-tina-field={tinaField(page, 'costsOutro')}
                        dangerouslySetInnerHTML={{ __html: costsOutro }}
                      />
                    )}
                  </div>
                </FadeIn>
                ...
                <FadeIn delay={0.1}>
                  <div>
                    {multimodalLabel && (
                      <p
                        className="gama-label mb-3 text-xs font-bold tracking-[0.2em] uppercase"
                        data-tina-field={tinaField(page, 'multimodalLabel')}
                      >
                        {multimodalLabel}
                      </p>
                    )}
                    <h2
                      className="text-3xl font-bold md:text-4xl"
                      data-tina-field={tinaField(page, 'multimodalTitle')}
                    >
                      {multimodalTitle}
                    </h2>
                    <StaggerContainer
                    ...
                    >
                      {multimodal.map((m: any, i: number) => (
                        <StaggerItem
                          key={i}
                          className="group relative rounded-2xl border border-white/[0.08] bg-white/[0.03] p-6 transition-all duration-300 hover:border-white/20 hover:bg-white/[0.06]"
                        >
                          {m.icon && (
                            <div className="mb-3 text-2xl" data-tina-field={tinaField(m, 'icon')}>
                              {m.icon}
                            </div>
                          )}
                          <h3
                            className="mb-2 text-lg font-bold"
                            data-tina-field={tinaField(m, 'title')}
                          >
                            {m.title}
                          </h3>
                          <p
                            className="gama-body text-sm leading-relaxed"
                            data-tina-field={tinaField(m, 'description')}
                          >
                            {m.description}
                          </p>
                        </StaggerItem>
                    ...
                    {multimodalDescription && (
                      <p
                        className="gama-body mt-6 text-base"
                        data-tina-field={tinaField(page, 'multimodalDescription')}
                      >
                        {multimodalDescription}
                      </p>
                    )}
          ...
          <section className="py-16" style={{ background: bgSecondary }}>
            <div className="container mx-auto px-6 md:px-16">
              <FadeIn>
                ...
                {costsIntro && (
                  <p
                    className="gama-body mt-4 max-w-3xl text-base [&_a]:font-medium [&_a]:text-[#4c8dff] [&_a]:underline"
                    data-tina-field={tinaField(page, 'costsIntro')}
                    dangerouslySetInnerHTML={{ __html: costsIntro }}
                  />
                )}
              ...
              {costsOutro && (
                <FadeIn>
                  <p
                    className="gama-body mt-8 max-w-3xl text-base [&_a]:font-medium [&_a]:text-[#4c8dff] [&_a]:underline"
                    data-tina-field={tinaField(page, 'costsOutro')}
                    dangerouslySetInnerHTML={{ __html: costsOutro }}
                  />
                </FadeIn>
              ...
              >
                {multimodal.map((m: any, i: number) => (
                  <StaggerItem
                    ...
                    className="group relative rounded-2xl border border-white/[0.08] bg-white/[0.03] p-8 transition-all duration-300 hover:border-white/20 hover:bg-white/[0.06]"
                  >
                    <div className="mb-4 text-3xl" data-tina-field={tinaField(m, 'icon')}>
                      {m.icon}
                    </div>
                    <h3 className="mb-3 text-xl font-bold" data-tina-field={tinaField(m, 'title')}>
                      {m.title}
                    </h3>
                    <p
                      className="gama-body text-sm leading-relaxed"
            ...
            <div className="container mx-auto px-6 md:px-16">
              <FadeIn className="text-center">
                {faqLabel && (
                  <p
                    className="gama-label mb-3 text-xs font-bold tracking-[0.2em] uppercase"
                    data-tina-field={tinaField(page, 'faqLabel')}
                  >
                    {faqLabel}
                  </p>
                )}
                <h2
                  ...
                  data-tina-field={tinaField(page, 'faqTitle')}
                >
                  {faqTitle}
                </h2>
              </FadeIn>
              <FadeIn className="mx-auto mt-12 max-w-3xl">
                {faqs.map((f: any, i: number) => (
                  <details
                    key={i}
                    className="group border-b border-white/[0.1] py-5"
                    open={i === 0}
                  >
                    ...
                    <p
                      className="gama-body mt-4 text-base leading-relaxed"
                      data-tina-field={tinaField(f, 'answer')}
                    >
                      {f.answer}
                    </p>
                  </details>
                ...
                {cta?.badges && cta.badges.length > 0 ? (
                  ...
                  <div className="relative overflow-hidden py-8 text-center">
                    <div
                      ...
                      style={{
                        background:
                          'radial-gradient(ellipse 70% 60% at 50% 0%, rgba(255,176,32,0.10), transparent 60%)',
                      }}
                    />
                    <div className="relative">
                      {cta?.label && (
                        <p
                          className="gama-label mb-4 flex items-center justify-center gap-2 text-xs font-bold tracking-[0.2em] uppercase"
                      ...
                      <h2
                        className="mx-auto max-w-2xl text-4xl font-bold md:text-5xl"
                        data-tina-field={tinaField(cta, 'headline')}
                      >
                        {cta?.headline}
                      </h2>
                      <p
                        className="gama-body mx-auto mt-6 max-w-xl text-base"
                      ...
                      <div className="mt-10 flex flex-wrap justify-center gap-3.5">
                        ...
                        {cta?.secondaryBtn?.text && (
                          <Link
                            ...
                            className="inline-flex h-[50px] items-center rounded-lg border border-white/15 px-6 font-mono text-sm font-medium text-white transition hover:border-white/35"
                          >
                            {cta.secondaryBtn.text}
                          </Link>
                        )}
                      ...
                      <div className="mt-5 flex flex-wrap justify-center gap-2.5">
                        {cta.badges.map((b: any, i: number) => (
                          <Link
                            key={i}
                            href={b?.href ?? '#'}
                            data-tina-field={tinaField(b, 'text')}
                  ...
                  <div className="relative overflow-hidden rounded-3xl border border-white/[0.08] p-12 text-center md:p-20">
                    <div
                      ...
                      style={{
                        background: `linear-gradient(135deg, ${primaryColor}1A, transparent, ${accentColor}0F)`,
                      }}
                    />
                    <div
                      className="pointer-events-none absolute inset-0"
                      style={{
                        background: `radial-gradient(ellipse 60% 50% at 50% 50%, ${primaryColor}21 0%, transparent 70%)`,
                      }}
                    />
                    <div className="relative">
                      <h2
                      ...
                      <p
                        className="gama-body mx-auto mt-6 max-w-xl text-lg"
                        data-tina-field={tinaField(cta, 'subtext')}
                      >
                        {cta?.subtext}
                      </p>
      ...
      <footer
        ...
        style={{ background: bgFooter, fontFamily: `'${fontFamily}', sans-serif` }}
      >
        <div className={`mx-auto grid max-w-7xl grid-cols-1 gap-8 px-4 ${footerGridClass}`}>
          {hasFooterCta && (
            <div>
              <h2
                className="text-2xl leading-tight font-bold sm:text-3xl md:text-4xl"
                data-tina-field={tinaField(ftr, 'ctaHeadline')}
              >
                {ftr?.ctaHeadline}
              </h2>
              <p
                className="mt-6 text-sm font-normal sm:text-base"
                data-tina-field={tinaField(ftr, 'ctaDescription')}
              >
                {ftr?.ctaDescription}
              </p>
            </div>
                  ...
                  if (subLinks.length === 0) {
                    return (
                      <Link
                        ...
                        href={link.href}
                        aria-current={active ? 'page' : undefined}
                        className={`text-sm hover:underline sm:text-base ${active ? 'font-semibold text-[#00FCE2] underline' : 'font-normal'}`}
                        data-tina-field={tinaField(link, 'title')}
                      >
                        {link.title}
                      </Link>
                  ...
                  return (
                    <div key={i} className="flex flex-col gap-2">
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          type="button"
                          onClick={() => setOpenFooterNav(isOpen ? null : i)}
                          aria-expanded={isOpen}
                      ...
                      <div
                        className={`${isOpen ? 'flex' : 'hidden'} flex-col gap-2 border-l border-white/20 pl-3`}
                      >
                        {subLinks.map((sub: any, j: number) => (
                          <Link
                            key={j}
                            href={sub.href}
                            aria-current={isActive(sub?.href) ? 'page' : undefined}
                            className={`text-sm hover:underline sm:text-base ${isActive(sub?.href) ? 'font-semibold text-[#00FCE2]' : 'font-normal text-white/70 hover:text-white'}`}
                            data-tina-field={tinaField(sub, 'title')}
                          >
                            {sub.title}
                          </Link>
              ...
              <div key={i} className={i > 0 ? 'mt-8' : ''}>
                <div data-tina-field={tinaField(office, 'flagImage')} className="mb-3">
                  {office.flagImage ? (
                    ...
                    <div className="flex h-10 w-20 items-center justify-center rounded border border-dashed border-white/20 text-xs text-white/30">
                      + Flag
                    </div>
                  )}
                </div>
                <h3
                  ...
                  data-tina-field={tinaField(office, 'city')}
                >
                  {office.city}
                </h3>
                <p
                  className="mt-4 text-sm font-normal sm:text-base"
        ...
        <div className="mx-auto mt-12 flex max-w-7xl flex-col items-center justify-between gap-4 px-4 md:flex-row">
          <p className="text-center text-sm font-normal text-white underline">
            {ftr?.legalLinks
              ...
              .map((link: any, i: number) => (
                <span key={link.href}>
                  {i > 0 && ' | '}
                  <Link href={link.href} data-tina-field={tinaField(link, 'title')}>
                    {link.title}
                  </Link>
                </span>
              ))}{' '}
            | © <span data-tina-field={tinaField(ftr, 'copyrightName')}>{ftr?.copyrightName}</span>{' '}
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex gap-2">
              ...
              {ftr?.social?.linkedin && (
                <SocialIcon kind="linkedin" href={ftr.social.linkedin} size={5} />
              )}
              {ftr?.social?.facebook && (
                <SocialIcon kind="facebook" href={ftr.social.facebook} size={5} />
              )}
              {ftr?.social?.youtube && (
                <SocialIcon kind="youtube" href={ftr.social.youtube} size={5} />
              )}
            </div>
          </div>
        </div>
